...
static int ws_panel_auto_detect(struct ws_panel *ts)
{
	...
	dev_info(&ts->i2c->dev, "automatic panel detect screen_type=0x%x rotate=0x%x fps=0x%x\n", screen_type, rotate, fps);
	...
	if (screen_type == 0x01) {
		...
		case 2:
			...
			ts->mode = fps == 60 ? &ws_panel_1200x1920_60fps_mode : &ws_panel_1200x1920_30fps_mode;
			break;
		case 1:
		case 3:
			dev_info(&ts->i2c->dev, "select 1920x1200 panel\n");
			ts->mode = fps == 60 ? &ws_panel_1920x1200_60fps_mode : &ws_panel_1920x1200_30fps_mode;
			break;
		default:
